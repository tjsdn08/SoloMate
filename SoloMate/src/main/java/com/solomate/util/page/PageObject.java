package com.solomate.util.page;
...
public class PageObject {
	...
	// 일반 게시판 / 공지사항 검색에 필요한 변수 선언
	private String key; // 검색할 항목
	private String word; // 사용자가 검색할 단어
	...
	// 페이지에 대한 정보를 계산하는 처리가되어있다. 반드시 실행해 주셔야만 한다. (데이터가 나오지 않는다.)
	...
	// 현재 페이지 정보가 없는 전달되는 다른 데이터 처리 - 리스트에서 다른 페이지로 갔다가 다시 돌아올 때 꼭 필요하다.
	...
	// 현재 페이지 정보가 포함된 전달되는 다른 데이터 처리 - 리스트에서 다른 페이지로 갔다가 다시 돌아올 때 꼭 필요하다.
	...
	public String toString() {
		return "PageObject [page=" + page + ", perPageNum=" + perPageNum + ", startRow=" + startRow + ", endRow="
				...
				+ ", period=" + period + ", accepter=" + accepter + ", acceptMode=" + acceptMode + ", no=" + no + "]";
	}
